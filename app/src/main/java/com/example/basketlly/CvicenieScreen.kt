...
class CvicenieScreen: AppCompatActivity() {
        ...
        binding.buttonAddExercise.setOnClickListener{
            ...
            if (sendNazov.isNotEmpty() && sendDane.toString().isNotEmpty() && sendPokusy.toString().isNotEmpty()
                ...
                Intent(this, TreningScreen::class.java).also {
                    it.putExtra("EXTRA_SENDNAZOV", sendNazov)
                    it.putExtra("EXTRA_SENDDANE", sendDane.toInt())
                    it.putExtra("EXTRA_SENDPOKUSY", sendPokusy.toInt())
    ...
    private fun percentoStrelby() {
        ...
        if (pokusyHodnota == 0) {val toast = Toast.makeText(this, "Division by zero!", Toast.LENGTH_SHORT)
            toast.show()}
